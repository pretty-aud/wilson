-- =============================================================================
-- 0000_rabbit_base_schema.sql
--
-- Canonical RABBIT v0.1 schema. This content previously lived only at
-- src/tools/rabbit_v0.1.0/db/schema.sql and was applied out-of-band on
-- wilson-dev during Session 1 setup. Promoting it to a Supabase migration
-- so fresh deployments (CI, wilson-staging, wilson-prod, new devs) get the
-- base RABBIT tables automatically before the multi-user migrations
-- (0001+) start ALTERing them.
--
-- Prefix 0000 sorts before 0001_workspaces_and_users.sql so this runs first
-- on a clean database. Fully idempotent (IF NOT EXISTS / ON CONFLICT / etc.)
-- so pushing it to an environment that already had schema.sql applied is a
-- safe no-op.
--
-- The standalone schema.sql file is retained as a reference copy for
-- RABBIT-only standalone installs (Electron local IndexedDB → PostgREST test
-- harness) that don't use Supabase migrations. Keep the two in sync if you
-- touch either one.
-- =============================================================================
