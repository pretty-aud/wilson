...
- **Authentication events have a server-side writer since 0070 (Track B bundle B2, part 1), but nothing shows them yet and the hooks that feed it are a per-project dashboard switch.** Measured first: hosted GoTrue writes nothing to `auth.audit_log_entries` (empty on dev and staging beside hundreds of live sessions), so the stream the brief hoped to read does not exist. `public.auth_events` now takes sign-in and MFA success/failure rows from two Supabase Auth hooks (`hook_password_verification_attempt`, `hook_mfa_verification_attempt`), operator sign-ins included, and has a stamped, RLS-guarded write path for the client's own sign-in, sign-out, idle-timeout and session-cap rows. Still true until B2 part 2: the hooks are inert until enabled in each project's dashboard (OWED_AUDREY §14); no screen lists the rows (the Admin Terminal "Sign-ins" view and the operator mirror are unbuilt); the app emits no sign-out / idle / cap rows yet, so `WIL-1002` is still unwired; guard refusals are still unlogged; hook rows carry no client address (GoTrue passes none).
