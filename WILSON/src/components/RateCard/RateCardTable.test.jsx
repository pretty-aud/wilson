...
describe('both branches of every inline editor survive (review Risk 1)', () => {
  ...
  it('gives the %/$ toggle a real target in BOTH branches (F-R21)', () => {
    ...
    expect(css).toMatch(/\.rc-comp-type \{[^}]*flex: 0 0 var\(--control-sm\);/s)
    expect(css).toMatch(/\.rc-comp \{ display: flex; align-items: center; justify-content: flex-end; gap: 8px; \}/)
...
describe('the totals row', () => {
  it('totals PER CURRENCY, never one figure across several', () => {
    mount()
    const totalRows = [...document.querySelectorAll('tr.rc-total-row')]
    // The fixture holds USD and GBP, so one summed figure would be a number
    // that is true of nothing — the fabricated-figure defect Q22 removed from
    // O.T.T.E.R.
    expect(totalRows).toHaveLength(2)
    const labels = totalRows.map(r => r.querySelector('.rc-total-label').textContent)
    expect(labels[0]).toMatch(/GBP/)
    expect(labels[1]).toMatch(/USD/)
    // …and each says how many rates it covers, so a partial total cannot read
    // as a whole one.
    expect(labels[1]).toMatch(/2 rates/)
    expect(labels[0]).toMatch(/1 rate\b/)
  })

  it('sums the day rate and the computed total of its own currency only', () => {
    mount()
    const usd = [...document.querySelectorAll('tr.rc-total-row')]
      .find(r => r.textContent.includes('USD'))
    const cells = usd.querySelectorAll('td')
    // Producer 800 + Editor 600 = 1400 day.
    expect(cells[1].textContent).toMatch(/1,400/)
    // Producer: 800 + 20% dept burden + 10% dept overhead = 1040.
    // Editor:   600 + 15% burden (90) + 50 fixed overhead  =  740.
    expect(cells[3].textContent).toMatch(/1,780/)
  })

  it('renders no totals row when nothing has a rate', () => {
    mount({ entries: [] })
...
describe('the row actions and the department bar', () => {
  it('puts duplicate and delete in a reserved slot revealed on hover and focus', () => {
    mount()
  ...
  it('indents the department bar to the first column\'s text inset', () => {
    ...
    expect(css).toMatch(/\.rc-dept \{[^}]*padding: 0 var\(--cell-pad-x\);/s)
    expect(css).toMatch(/\.rc-dept-cell \{ padding: 0;/)
  })
