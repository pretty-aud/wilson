  ...
  return (
    <div>
      ...
      <div className="relative flex items-center gap-0 px-4 py-2" style={{ borderBottom: '1px solid #44403c' }}>
        ...
        <span className="flex-[2] text-label uppercase font-semibold" style={{ color: '#78716c' }}>Name</span>
        <span className="w-28 text-label uppercase font-semibold text-center" style={{ color: '#78716c' }}>Status</span>
        <span className="w-16 text-label uppercase font-semibold text-center" style={{ color: '#78716c' }}>Assets</span>
        <span className="w-16 text-label uppercase font-semibold text-center" style={{ color: '#78716c' }}>Tasks</span>
        <span className="flex-[2] text-label uppercase font-semibold" style={{ color: '#78716c' }}>Description</span>
        <span className="w-20" />
      ...
      {levels.map(lv => (
        <div key={lv.id}
          ...
          <div className="px-3 py-2.5 flex flex-col gap-1">
            <span className="text-dense truncate font-semibold" style={{ color: '#e7e5e4' }}>
            ...
            {lv.description && (
              <span className="text-dense truncate" style={{ color: '#78716c' }}>
                {lv.description}
              </span>
            )}
          </div>
...
// ─── Level detail popup ───
function LevelDetailPopup({ levelId, ctx, assetCountByLevel, taskCountByLevel, projectMembers, roleEntries, thumbRevision, onThumbChanged, onClose, onRequestDelete }) {
  ...
  const hasThumbnail = !!level.thumbnail_image
  ...
  async function handleSetThumbnail() {
    ...
    try {
      await window.electronAPI.rabbit.generateEntityThumbnail({ entityType: 'level', entityId: level.id, sourcePath: imagePath })
    } catch (e) { console.error('level thumbnail gen failed:', e) }
    onThumbChanged?.()
  }

  async function handleClearThumbnail() {
    handleUpdate({ thumbnail_image: null })
    try { await window.electronAPI.rabbit.clearEntityThumbnail({ entityType: 'level', entityId: level.id }) } catch {}
    onThumbChanged?.()
  }

  async function handleCreateTask(draft) {
    try {
      await ctx?.addTask?.({ ...draft, level_id: level.id })
  ...
  return (
    <>
      ...
      <div className="fixed z-50 inset-0 flex items-center justify-center gap-3 pointer-events-none">
        ...
        <div className="flex-1 overflow-auto flex">
          ...
          <div className="flex items-start gap-4 mb-5">
            <div className="relative flex-shrink-0 rounded-control overflow-hidden flex items-center justify-center group/thumb"
              ...
              {hasThumbnail ? (
                <>
                  <img
                    src={`/api/rabbit/projects/${project?.id}/levels/${level.id}/thumbnail?r=${thumbRevision}`}
                ...
                <button type="button" onClick={handleSetThumbnail}
                  ...
                  <Gamepad2 className="w-5 h-5 group-hover/thumb:opacity-0 transition-opacity absolute" style={{ color: '#292524' }} />
                </button>
              )}
            </div>
          ...
          <div className="grid grid-cols-2 gap-x-6 gap-y-4 mb-5">
            ...
            <div>
              <label className="block text-label uppercase mb-1" style={{ color: '#78716c' }}>Start Date</label>
              <input
                type="date"
            ...
            <div>
              <label className="block text-label uppercase mb-1" style={{ color: '#78716c' }}>Due Date</label>
              <input
                type="date"
      ...
      </div>{/* close flex centering container */}
    </>
  )
}
    ...
    <>
      ...
      <div className="fixed z-[60] top-1/2 left-1/2 w-full max-w-sm rounded-control overflow-hidden"
        ...
        <div className="px-5 py-4">
          <p className="text-dense leading-relaxed" style={{ color: '#a8a29e' }}>{message}</p>
        </div>
...
function InlineText({ value, placeholder, onCommit, size = 'md' }) {
  ...
  const textSize = size === 'sm' ? 'text-dense' : 'text-dense'
  ...
  return (
    <span onClick={() => setEditing(true)}
      className={`${textSize} truncate cursor-pointer hover:text-orange-400 transition-colors block`}
...
function FieldLabel({ children }) {
  return (
    <label className="block text-label uppercase font-semibold mb-1.5" style={{ color: '#78716c' }}>
      {children}
    </label>
  )
}
