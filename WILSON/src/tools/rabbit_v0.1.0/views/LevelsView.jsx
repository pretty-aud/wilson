      ...
      {showSaveDialog && (
        <div className="px-4 py-3 flex items-center gap-2" style={{ borderBottom: '1px solid #44403c', backgroundColor: '#1c1917' }}>
          <input type="text" value={saveName} onChange={e => setSaveName(e.target.value)} placeholder="View name..."
            className="px-2.5 py-1.5 text-dense font-mono rounded focus:ring-2 focus:ring-orange-500 w-48"
      ...
      {levels.map(lv => (
        <div key={lv.id}
          ...
          <div className="px-3 py-2.5 flex flex-col gap-1">
            ...
            {lv.description && (
              <span className="text-dense font-mono truncate" style={{ color: '#78716c' }}>
                {lv.description}
              </span>
            )}
          </div>
          ...
          <div className="grid grid-cols-2 gap-x-6 gap-y-4 mb-5">
            ...
            <div>
              ...
              <input
                ...
                className="w-full px-2.5 py-1.5 text-dense font-mono rounded-sm focus:ring-2 focus:ring-orange-500"
                style={{
            ...
            {editingDesc ? (
              <div>
                <textarea value={descDraft} onChange={e => setDescDraft(e.target.value)}
                  className="w-full px-3 py-2 text-dense font-mono rounded resize-none focus:ring-2 focus:ring-orange-500"
              ...
              <div onClick={() => setEditingDesc(true)}
                className="px-3 py-2 text-dense font-mono rounded cursor-pointer hover:bg-stone-800 transition-colors"
    ...
    <>
      ...
      <div className="fixed z-[60] top-1/2 left-1/2 w-full max-w-sm rounded overflow-hidden"
        ...
        <div className="px-5 py-4">
          <p className="text-dense font-mono leading-relaxed" style={{ color: '#a8a29e' }}>{message}</p>
        </div>
        ...
        <div className="flex-1 overflow-auto px-5 py-4 flex flex-col gap-4">
          ...
          <div>
            ...
            <textarea value={description} onChange={e => setDescription(e.target.value)} rows={3}
              className="w-full px-2.5 py-1.5 text-dense font-mono rounded focus:ring-2 focus:ring-orange-500 resize-none"
  ...
  if (editing) {
    return (
      <input ref={inputRef} type="text" value={draft}
        ...
        onBlur={commit}
        onKeyDown={e => { if (e.key === 'Enter') commit(); if (e.key === 'Escape') { setDraft(value); setEditing(false) } }}
        className="w-full text-body font-mono px-2.5 py-1.5 rounded focus:ring-2 focus:ring-orange-500"
        style={{ backgroundColor: '#1c1917', color: '#f4a261', border: '1px solid #44403c' }} />
    )
  }
  return (
    <div onClick={() => setEditing(true)}
      className="text-dense font-mono px-2.5 py-1.5 rounded cursor-pointer hover:bg-stone-800 transition-colors"
