      ...
      <div className="flex items-center gap-2 px-4 py-2 flex-wrap flex-shrink-0" style={{ borderBottom: '1px solid #44403c' }}>
        ...
        <div className="flex items-center gap-2 ml-auto">
          ...
          <button
            ...
            className="flex items-center gap-1 px-2 py-1 text-dense rounded-control transition-colors hover:bg-stone-800 disabled:opacity-40"
            ...
            <div key={c.key} className="flex items-center gap-2 px-3.5 py-2" style={{ flex: c.flex, minWidth: 0 }}>
              ...
              {editTitle && !isProjectBound ? (
                <input type="text" value={localTitle} autoFocus
                  ...
                  className="flex-1 px-1 py-0.5 text-dense font-semibold rounded-control focus:ring-1 focus:ring-amber-500"
              ...
              {isProjectBound && (
                <span className="text-label uppercase px-1.5 py-0.5 rounded-control" style={{ color: '#78716c', backgroundColor: '#292524', border: '1px solid #3a3733' }}>bound</span>
              )}
            </div>
          )
        }
              ...
              {c.key === 'start_date' ? (
                editDate && !isProjectBound ? (
                  <input type="date" value={localDate} autoFocus
                    ...
                    className="w-full px-1 py-0.5 text-dense rounded-control focus:ring-1 focus:ring-amber-500"
                    style={{ backgroundColor: '#1c1917', color: '#f59e0b', border: '1px solid #44403c' }}
                  />
                ) : (
                  <span
                    className="text-dense font-mono cursor-pointer"
                    style={{ color: '#f59e0b' }}
      ...
      <div className="px-3.5 py-3 flex items-center justify-between"
        ...
        <GatedAction allowed={canWrite}>
          <button type="button" onClick={() => onAddTask(groupDefaults())}
            className="p-1 rounded-control hover:bg-stone-600 transition-colors" style={{ color: '#a8a29e' }}>
      ...
      <div className="flex-1 overflow-y-auto p-2.5 flex flex-col gap-2" style={{ minHeight: 60 }}>
        ...
        <GatedAction allowed={canWrite} display="block">
          <input ref={inputRef} type="text" value={addTitle} onChange={e => setAddTitle(e.target.value)}
            ...
            onBlur={commitAdd}
            className="w-full px-3 py-2 text-dense rounded-control focus:ring-1 focus:ring-orange-500 transition-shadow"
            style={{
              color: '#a8a29e',
