  ...
  return (
    <div className="h-full flex flex-col" style={{ backgroundColor: '#1c1917' }}>
      ...
      <div className="flex items-center gap-3 px-4 py-2.5 flex-wrap" style={{ borderBottom: '1px solid #44403c', backgroundColor: '#292524' }}>
        ...
        {/* Right: count, thumb size, add asset */}
          ...
          groups.map(g => (
            <div key={g.key}>
              <div className="flex items-center gap-2 px-5 py-2.5 cursor-pointer hover:bg-stone-800/30 transition-colors"
                ...
                <span className="text-[12px] font-mono uppercase tracking-wider font-bold" style={{ color: groupAccent(groupBy, g.key) }}>
                  {g.label}
                </span>
  ...
  if (assets.length === 0 && (!groups || groups.length === 0)) {
    return (
      <div className="h-full flex flex-col items-center justify-center gap-3">
        ...
        <span className="text-[13px] font-mono" style={{ color: '#78716c' }}>No assets yet</span>
        <span className="text-[11px] font-mono" style={{ color: '#57534e' }}>Click "New asset" to get started</span>
      </div>
    )
  }
            ...
            <div key={g.key}>
              ...
              <div className="flex items-center gap-2 px-3.5 py-2.5 cursor-pointer hover:bg-stone-800/30 transition-colors"
                ...
                {collapsed
                  ? <ChevronRight className="w-3.5 h-3.5" style={{ color: '#78716c' }} />
                  : <ChevronDown className="w-3.5 h-3.5" style={{ color: '#78716c' }} />}
                <span className="text-[12px] font-mono uppercase tracking-wider font-bold" style={{ color: accent }}>
                  {g.label}
                </span>
                <span className="text-[10px] font-mono" style={{ color: '#78716c' }}>
                  ({g.assets.length})
...
function AssetRow({ asset, columns, phases, phaseLabel, taskCount, warning, rowH, thumbSize, thumbRevision, onUpdate, onDelete, onWarningClick, onDetailClick, onThumbChanged }) {
  ...
  function renderCell(col) {
    switch (col.key) {
      ...
      case 'start_date':
        return (
          <input
            ...
            className="px-1 py-0.5 text-[11px] font-mono rounded focus:outline-none focus:ring-2 focus:ring-orange-500 w-full hover:bg-stone-700/40 transition-colors"
      ...
      case 'description':
        return (
          <span className="text-[11px] font-mono truncate block" style={{ color: '#a8a29e' }}
            title={asset.description || ''}>
            {asset.description || '\u2014'}
          </span>
        )
      case 'tasks':
        return <span className="text-[11px] font-mono" style={{ color: '#a8a29e' }}>{taskCount}</span>
      case '_actions':
        return (
      ...
      default:
        return <span className="text-[11px] font-mono" style={{ color: '#a8a29e' }}>{asset[col.key] ?? '--'}</span>
    }
  }
...
function AssetGallery({ assets, phaseById, taskCountByAsset, ctx, thumbRevision, onThumbChanged, onWarningClick, onDetailClick }) {
  if (assets.length === 0) {
    return (
      <div className="h-full flex flex-col items-center justify-center gap-3">
        <Boxes className="w-12 h-12" style={{ color: '#44403c' }} />
        <span className="text-[13px] font-mono" style={{ color: '#78716c' }}>No assets</span>
      </div>
    )
  }
  return (
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div
          className="absolute top-1.5 left-1.5 px-1.5 py-0.5 text-[9px] font-mono uppercase tracking-wider rounded"
      ...
      >
        ...
        <div className="flex-1 overflow-auto px-5 py-4">
          <div className="space-y-4">
            ...
            {/* Name */}
            <div>
              <label className="text-[9px] font-mono uppercase tracking-wider mb-1 block" style={{ color: '#78716c' }}>Name *</label>
              <input
                autoFocus
                type="text"
                value={draft.name}
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="text-[9px] font-mono uppercase tracking-wider mb-1 block" style={{ color: '#78716c' }}>Type</label>
              ...
              <div>
                <label className="text-[9px] font-mono uppercase tracking-wider mb-1 block" style={{ color: '#78716c' }}>Status</label>
            ...
            {/* Phase */}
            <div>
              <label className="text-[9px] font-mono uppercase tracking-wider mb-1 block" style={{ color: '#78716c' }}>Phase</label>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="text-[9px] font-mono uppercase tracking-wider mb-1 block" style={{ color: '#78716c' }}>Start Date</label>
              ...
              <div>
                <label className="text-[9px] font-mono uppercase tracking-wider mb-1 block" style={{ color: '#78716c' }}>Due Date</label>
            ...
            {/* Description */}
            <div>
              <label className="text-[9px] font-mono uppercase tracking-wider mb-1 block" style={{ color: '#78716c' }}>Description</label>
              <textarea
                value={draft.description}
                onChange={e => patch({ description: e.target.value })}
                rows={2}
                placeholder="Optional description..."
                className="w-full px-3 py-2 text-[11px] font-mono rounded focus:outline-none focus:ring-2 focus:ring-orange-500 resize-y"
                style={{ backgroundColor: '#1c1917', color: '#d6d3d1', border: '1px solid #44403c' }}
              />
            </div>
            ...
            <div>
              <label className="text-[9px] font-mono uppercase tracking-wider mb-1 block" style={{ color: '#78716c' }}>Task Template</label>
              <select
            ...
            {previewTasks.length > 0 && (
              <div className="rounded overflow-hidden" style={{ border: '1px solid #44403c' }}>
                <div className="px-3 py-1.5" style={{ backgroundColor: '#1c1917', borderBottom: '1px solid #44403c' }}>
                  <span className="text-[9px] font-mono uppercase tracking-wider font-bold" style={{ color: '#fb923c' }}>
                ...
                <div style={{ maxHeight: 140, overflow: 'auto' }}>
                  {previewTasks.map((t, i) => (
                    <div key={t.id || i} className="flex items-center px-3 py-1.5"
                      ...
                      <span className="text-[11px] font-mono flex-1 truncate" style={{ color: '#d6d3d1' }}>
                        {t.name || 'Untitled'}
                      </span>
                      {t.role_slug && (
                        <span className="text-[9px] font-mono px-1.5 py-0.5 rounded mr-2" style={{ color: '#78716c', backgroundColor: '#292524' }}>
                          {fmt(t.role_slug)}
                        </span>
                      )}
                      <span className="text-[10px] font-mono font-bold flex-shrink-0" style={{ color: '#f4a261' }}>
                        {t.bid_days || 0}d
                      </span>
                    </div>
                  ))}
        ...
        <div className="flex items-center justify-between px-5 py-3" style={{ borderTop: '1px solid #44403c' }}>
          ...
          <div className="flex items-center gap-2">
            ...
            <button type="button" onClick={handleConfirm}
              ...
              className="px-4 py-1.5 text-[11px] font-mono font-bold uppercase tracking-wider rounded transition-colors disabled:opacity-40"
              style={{ color: '#fff7ed', backgroundColor: '#ea580c', border: '1px solid #c2410c' }}>
...
function AssetDetailPopup({ asset, tasks, phase, ctx, thumbRevision, onThumbChanged, onClose }) {
  ...
  return (
    <>
      ...
      {/* Modal */}
      <div
        className="fixed z-50 top-1/2 left-1/2 w-full max-w-4xl rounded overflow-hidden flex flex-col"
        ...
        <div className="flex-1 overflow-auto px-5 py-4">
          ...
          <div className="flex items-start gap-5 mb-6">
            ...
            <div className="flex flex-col gap-1.5 pt-1">
              <div className="text-[9px] font-mono uppercase tracking-wider" style={{ color: '#78716c' }}>
                Thumbnail
              </div>
              <div className="flex items-center gap-2">
                <button type="button" onClick={handleSetThumbnail}
                  className="flex items-center gap-1 px-2 py-1 text-[11px] font-mono rounded hover:bg-stone-700 transition-colors"
              ...
              {hasThumbnail && (
                <span className="text-[10px] font-mono truncate max-w-[280px]" style={{ color: '#78716c' }}
          ...
          {tasks.length === 0 ? (
            <div className="text-[11px] font-mono italic py-4 text-center" style={{ color: '#78716c' }}>
...
function PropField({ label, value, children }) {
  return (
    <div>
      <div className="text-[9px] font-mono uppercase tracking-wider mb-0.5" style={{ color: '#78716c' }}>
        {label}
      </div>
      {children || (
...
function Th({ children, style: extraStyle }) {
  return (
    <th
      className="px-3 py-2.5 text-[11px] font-mono uppercase tracking-wider font-semibold text-left"
  ...
  if (editing) {
    return (
      <input autoFocus value={draft} onChange={e => setDraft(e.target.value)}
        ...
        className="w-full px-1.5 py-1 text-[11px] font-mono rounded focus:outline-none focus:ring-2 focus:ring-orange-500"
        style={{ backgroundColor: '#1c1917', color: '#f4a261', border: '1px solid #44403c' }} />
    )
  }
  return (
    <button type="button" onClick={() => { setDraft(value); setEditing(true) }}
      className="text-[11px] font-mono text-left w-full truncate hover:bg-stone-700/40 px-1.5 py-1 rounded transition-colors"
