...
export default function AddFilesDialog({ bin, plan, scenes, onConfirm, onCancel, busy, progress, error = null }) {
  ...
  return (
    <Modal title={`Add to "${bin?.name || 'bin'}"`} onClose={onCancel} onBeforeClose={() => !dirty || window.confirm('Discard this batch? Your ticks, names and batch fields will be lost.')} width={860} busy={busy} error={error}
      ...
      {plan?.truncated && (
        <div className="flex items-center gap-2 px-2 py-1.5 mb-3 rounded-sm text-dense font-mono" style={{ color: C.amber, border: `1px solid ${C.amber}55`, backgroundColor: 'rgba(245,158,11,0.08)' }}>
      ...
      <div className="rounded-sm overflow-hidden" style={{ border: `1px solid ${C.line}` }}>
        ...
        <div className="max-h-[46vh] overflow-y-auto">
                ...
                <div className="px-1 min-w-0">
                  {sug ? (
                    <label className="flex items-center gap-1.5 cursor-pointer min-w-0">
                      ...
                      <span className="truncate text-dense font-mono" style={{ color: it.apply ? C.accentText : C.dim }} title={sug}>{sug}</span>
                    </label>
                  ) : <span className="text-dense font-mono" style={{ color: C.dimmer }}>—</span>}
                </div>
          ...
          {items.length === 0 && <div className="px-3 py-4 text-dense font-mono" style={{ color: C.dimmer }}>Nothing to add.</div>}
        </div>
      </div>
      <div className="mt-2 text-dense font-mono leading-relaxed" style={{ color: C.dimmer }}>
