...
export function Kbd({ children }) {
  return (
    <kbd className="inline-block px-1 rounded-sm text-dense font-mono leading-[14px]"
        ...
        : it.header
          ...
          : (
            <button key={i} type="button" disabled={it.disabled}
              ...
              className="w-full flex items-center gap-2 px-3 py-1.5 text-left text-dense font-mono hover:bg-stone-700 transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
      ...
      <div className="rounded-sm shadow-2xl flex flex-col" style={{ width, maxWidth: '94vw', maxHeight: '88vh', backgroundColor: C.bg, border: `1px solid ${C.line}` }}
        ...
        {(footer || error) && (
          <div className="flex items-center justify-end gap-2 px-4 py-3 flex-shrink-0 flex-wrap" style={{ borderTop: `1px solid ${C.line}` }}>
            {error && <span role="alert" className="text-dense font-mono mr-auto flex items-center gap-1.5 min-w-0" style={{ color: '#fca5a5' }}><AlertTriangle className="w-3 h-3 flex-shrink-0" /> <span className="truncate" title={String(error)}>{String(error)}</span></span>}
            {footer}
          </div>
        )}
      </div>
...
export function Field({ label, children, hint, inline = false, mixed = false }) {
  return (
    <label className={`flex ${inline ? 'items-center gap-3' : 'flex-col gap-1'} min-w-0`}>
      ...
      {hint && <span className="text-dense font-mono" style={{ color: C.dimmer }}>{hint}</span>}
    </label>
  )
}
...
export function Toggle({ checked, onChange, label }) {
  return (
    <label className="inline-flex items-center gap-2 cursor-pointer select-none">
      ...
      {label && <span className="text-dense font-mono" style={{ color: C.text }}>{label}</span>}
    </label>
  )
}
