    ...
    <div className="h-full overflow-auto" style={{ backgroundColor: '#1c1917' }}>
      <div className="p-6 flex flex-col gap-4">
        ...
        <div className="rounded-sm px-5 py-3.5" style={{ backgroundColor: '#0c0a09', border: '1px solid #292524' }}>
          ...
          {allProjects.length === 0 ? (
            <span className="text-dense font-mono italic" style={{ color: '#57534e' }}>No projects yet.</span>
          ) : (
        ...
        <Card>
          <div className="flex items-start justify-between gap-3">
            <div className="flex-1 min-w-0">
              ...
              {project.description && (
                <p className="text-dense font-mono mt-1.5 leading-relaxed" style={{ color: '#a8a29e' }}>
                  {project.description}
                </p>
              )}
              {/* Status + dates row */}
          ...
          <div className="flex items-center gap-2 mt-3 pt-3" style={{ borderTop: '1px solid #44403c' }}>
            ...
            {project.folder_root ? (
              ...
              <span className="text-dense font-mono italic" style={{ color: '#57534e' }}>
          ...
          {folderMsg && (
            <p className="text-dense font-mono mt-1.5" style={{ color: '#f87171' }}>
              {folderMsg}
            </p>
          )}
        </Card>
          ...
          {phases.length === 0 ? (
            ...
            <div className="flex flex-wrap gap-2">
              ...
              {phaseAssetCounts['__unphased__'] > 0 && (
                ...
                >
                  <span className="text-dense font-mono italic" style={{ color: '#a8a29e' }}>
            ...
            <div className="mt-4 flex flex-col gap-1">
              ...
              {Object.values(budget.byRole)
                ...
                .map(row => (
                  <div
                    key={row.role}
                    className="flex items-center justify-between text-dense font-mono px-2.5 py-1.5 rounded-sm"
        ...
        <div className="px-6 py-4 flex flex-col gap-4" style={{ backgroundColor: '#1c1917' }}>
          ...
          <div className="pt-3 mt-1" style={{ borderTop: '1px solid #44403c' }}>
            ...
            <div className="flex items-center justify-between px-3 py-2.5 mt-3 rounded-md" style={{ backgroundColor: '#1c1917', border: '1px solid #44403c' }}>
              <span className="text-dense font-mono" style={{ color: '#a8a29e' }}>Uses real-time engine</span>
...
function ProjectFilesSection({ files, managedFiles, ctx, project, update }) {
  ...
  return (
    <SettingsSection title="Files & Storage" icon={FolderOpen} accent={SECTION_ACCENT}>
      ...
      {uploadError && (
        <p className="text-dense font-mono leading-relaxed" style={{ color: '#ef4444' }}>
          {uploadError}
        </p>
      )}
...
function SettingsInput({ value, onChange, placeholder, size }) {
  ...
  return (
    <input
      ...
      className={`w-full font-mono rounded-md focus:ring-1 focus:ring-orange-500/50 transition-colors ${lg ? 'px-4 py-2.5 text-body font-semibold' : 'px-3 py-2 text-dense'}`}
...
function SettingsTextarea({ value, onChange, placeholder }) {
  return (
    <textarea
      ...
      placeholder={placeholder}
      rows={2}
      className="w-full px-3 py-2 text-dense font-mono rounded-md resize-y focus:ring-1 focus:ring-orange-500/50 transition-colors"
      style={{ backgroundColor: '#1c1917', border: '1px solid #44403c', color: '#d6d3d1' }}
    />
  )
}
...
function SettingsSelect({ value, options, labels, onChange, allowEmpty }) {
  return (
    <select
      value={value} onChange={e => onChange(e.target.value)}
      className="w-full px-3 py-2 text-dense font-mono rounded-md focus:ring-1 focus:ring-orange-500/50 transition-colors"
      style={{ backgroundColor: '#1c1917', border: '1px solid #44403c', color: '#d6d3d1' }}
    >
...
function ProjectMiniCard({ project, active, onClick }) {
  ...
  return (
    ...
    >
      ...
      {project.description && (
        <p
          className="text-dense font-mono leading-relaxed line-clamp-2"
          style={{ color: '#a8a29e' }}
        >
          {project.description}
        </p>
...
function ListRow({ title, tag, hint, danger }) {
  return (
    ...
    >
      ...
      <span className="flex-1 text-dense font-mono truncate" style={{ color: '#d6d3d1' }}>{title}</span>
      {hint && (
        <span className="text-dense font-mono italic truncate max-w-[140px]" style={{ color: '#78716c' }}>
          {hint}
        </span>
      )}
      {tag && (
