...
describe('the stylesheets: the rows that must be zero (T3)', () => {
  ...
  const JUDGED_BORDERS = [
    ...
    /* B3, 2026-09-24 — the Timeline's stage-1 sheet: four shape indicators,
       ...
       name and its chevron; a 2px edge there is a defect again), and so did
       the phase bar's 2px and its ghost's (a phase bar is told by that band,
       its taller box, its 600 label and its palette; its ghost by its
       taller box). The minimap frame's two sides are the Timeline's last. */
  ...
  it('CONTROL: the CSS scan is actually reading the stylesheets', () => {
    ...
    // (2026-09-25); 13: B5 rabbitBudget.css (2026-09-26).
    expect(CSS_FILES.length).toBe(13);
    ...
    expect(CSS_FILES).toContain('src/tools/rabbit_v0.1.0/views/rabbitBudget.css');
