...
| CI | **green on `4d16f59`, all four jobs** — pgTAP (Supabase local DB), Playwright auth (wilson-dev), issue-session smoke (wilson-dev), Vitest (renderer units). Run [34182128665](https://github.com/pretty-aud/wilson/actions/runs/34182128665). `04324a3` was green too (run 34181271227). **Only the one docs commit recording this run lands after it**, as in A4 | GitHub REST by FULL SHA, **jobs enumerated** rather than the run conclusion trusted — the run-level call returned success while the jobs call was still rate-limited and empty, so one alone is not proof |
