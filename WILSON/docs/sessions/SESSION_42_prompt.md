# SESSION 42 launch prompt — GOOGLE DRIVE (THE THIRD BYO PROVIDER)

> **§4a2 + §4a2b of `docs/NETWORK_STORAGE_DESIGN.md`.** The third member of
> the BYO storage family, after `network` (NAS, S34) and `s3` (S41). For the
> customer with no office server and no bucket, but a Google Workspace they
> already pay for.

> 🚨 **BLOCKED BY S40** (the provider registry) **and SEQUENCED AFTER S41.**
> S41 is not a technical dependency — it is the cheaper provider, so it
> proves the registry's shape without a third-party approval in the critical
> path. If S41 found the interface wanting, fix it there, not here.
>
...
> their schedule. **Start the verification submission BEFORE the build
> session, not during it** — see §1. Everything else here is ordinary work;
> this one item can idle for weeks.

> **STATE — re-measure, do not trust this block.** After S35 (2026-08-07):
> migrations **0000–0049**, next free **0050**. pgTAP **59 suites / 982
> assertions**, next suite **60**. Vitest **1022 / 47 files**. 🚨 S36–S41 land
...
3. **Read `docs/OUTSTANDING.md`** and **`docs/SYSTEMS_HANDBOOK.md` §12.1 +
   §12.7 + §17**.
4. **Read `NETWORK_STORAGE_DESIGN.md` §4a2 + §4a2b**, and the **S40 and S41
...
This session implements **S40's four functions** (`put`/`get`/`del`/`exists`)
...
- **The refresh token is the credential** — store it exactly as S41 stores
...
- 🚨 **The money gate cannot be expressed in Drive at all.** No path segment,
  ...
  **Financial files pin `storage_provider = 'supabase'`** — S40's invariant,
...
2. Sequence table + an S42 outcome block in `MASTER_PLAN_S19_ONWARD.md`.
...
6. Refresh the STATE block of `SESSION_43_prompt.md`; update the Claude
