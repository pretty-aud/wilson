# SESSION 41 launch prompt — S3-COMPATIBLE STORAGE (THE FIRST BYO PROVIDER)
...
> 🚨 **BLOCKED BY S40** (the provider registry). S40 builds
...
> assertions**, next suite **60**. Vitest **1022 / 47 files**. 🚨 S36–S40 all
> land before this one and will have moved every number. **Read the working
> tree.**
...
3. **Read `docs/OUTSTANDING.md`** and **`docs/SYSTEMS_HANDBOOK.md` §12.7 +
   §17**.
4. **Read `NETWORK_STORAGE_DESIGN.md` §4a2 + §4a2b** and the **S40 outcome
...
### 1. The config shape (into S40's `provider_config` JSONB)
...
- 🚨 **Resolve a file's provider FROM THE FILE ROW**, never from the
  ...
  orphan what it already wrote (S40 pins this; do not regress it).

### 5. Money stays home

**Financial files pin `storage_provider = 'supabase'` regardless of the
workspace's provider** — S40's invariant, enforced at the `uploadFile`
...
6. Refresh the STATE block of `SESSION_42_prompt.md`; update the Claude
