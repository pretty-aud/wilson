...
| ~~**S43**~~ ✅ **DONE (2026-08-10)** | **Design pass — NO migration, NO schema change, NO new pgTAP suite.** Phase A: company-first login, asterisk masking, invite-only onboarding (self-serve company creation REMOVED from the app surface, verified by BUNDLE), the phantom cursor, one auth field kit, the dead nav hover, and a post-sign-in WELCOME transition. Phase B: the near-white Team Members card replaced by a warm region, and **191 → 29** stone-ramp occurrences (the 29 remaining are all inside dark modals, where a grey is correct). 🚨 **The auth surfaces had been WHITE ON `#f4a261` — 2.06:1 — since Session 2**, and the page-transition title still was; both are now `#1c1917` at 8.49:1. Contrast is pinned executably with failing controls in `authContrast.test.js` + `lightSurface.test.js`. Commits `bec9185` → `17e2228` | 1 | ~~S42~~ ✅ | `SESSION_43_prompt.md` |
| **S43b** ⬅️ **NEW** | **Operator-sent invite link for a new company.** Audrey asked for the operator to email a setup link instead of handing over the show-once password. Split out of S43 §A5 because it needs a new `operator-workspaces` action and probably a `platform_audit` action-CHECK widening — i.e. **a migration**, which a design-polish session was barred from. Company creation itself already works operator-side | 1 | ~~S43~~ ✅ | [`SESSION_43b_prompt.md`](SESSION_43b_prompt.md) |
| **S38** ⏸️ **NEXT AND LAST** | see below | | | |
