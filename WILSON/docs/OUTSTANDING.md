...
⚠️ **Not a residual but a NEW CONSEQUENCE of the fix: a receipt is now exempt
from the Petal storage quota.** `rabbit_quota_exempt_path` (0055) exempts any
money segment from the RESTRICTIVE `petal_storage_quota_insert`, so a receipt
under `INVOICES/` can never be refused — while `workspace_petal_committed_bytes`
still counts its bytes against the allowance that refuses ordinary media. 0055
assumed money paths are tiny (one invoice PDF per budget line); the expense
picker takes `multiple` files with no `accept` and no size cap. Not a security
hole (0037 gates `expenses`), but a billing one. Bounding the exemption by size
is a 0055 change and a pricing decision — **Audrey's, handbook §12.9.**
