...
|---|---|---|
| Track C / **migration 0078** (2026-09-09) — the quota exemption bounded by size; pgTAP suite **77 extended** (probes 54-62), three breakers | **nothing.** | **One entry CLOSED: a receipt was exempt from the Petal storage quota at any size.** C4 made a receipt land under an `INVOICES/` segment so it would be money-gated, and that segment is what `rabbit_quota_exempt_path` (0055) keys on — so a receipt could not be refused however large, while `workspace_petal_committed_bytes` kept counting its bytes against the allowance that refuses ordinary media. Measured ceiling: the bucket's own **50 GiB** per object. Not a security hole (0037 gates `expenses`), a billing one. Audrey's ruling, asked and answered this session: **bound the exemption by size**, not cap the picker. 0078 adds `public.rabbit_quota_exempt_max_bytes()` = **25 MiB** as the one definition, plus `rabbit_quota_exempt_bytes(name,bytes)` and `rabbit_quota_exempt_object(name,md)` composed over it. 🚨 **THE HAND-OFF NAMED ONE ENFORCEMENT SITE AND THERE ARE TWO.** Besides the RESTRICTIVE `petal_storage_quota_insert`, `reserve_upload_bytes` (0073, C1) returned NULL early for any exempt path — and that is the one the CLIENT calls, from `supabaseProvider.js`, before any byte moves. Bounding only the policy would have let a 5 GB receipt reserve nothing, upload for ten minutes and be refused at commit, which is the exact failure C1 exists to remove. Both sites are bounded by one predicate. 🚨 **`COALESCE(bytes, 0)` is load-bearing and the polarity is the counter-intuitive one: an UNKNOWN size KEEPS the exemption**, because 0055's other two reasons (the `FINANCE/RATES.json` mirror and the `PROJECT.json` manifest, both rewritten constantly) are not about size, and a bare comparison would be NULL — which DENIES under a RESTRICTIVE policy, failing every such write with a symptom indistinguishable from the bound working. Safe because `completeUpload` writes storage-api's own `size`, so a forged tus `contentLength` only moves bytes before the refusal. **`rabbit_quota_exempt_path` is UNCHANGED and delegated to**, so suite 65's probes 13-17 stay green. **Proven by three breakers:** B1 (size axis dropped) reddens exactly 55, 60, 61 — the three refusal probes at both sites; B2 (COALESCE removed) reddens exactly 62; B3 (policy bounded, reservation not) reddens exactly 55, which is what proves the two-site claim. **Suite 77 probe 10 asserted "a 5 GB invoice is not refused"** — true until now, corrected in place rather than deleted. `StorageSection.jsx`'s two banners said money files and the manifest "still save"; they now say "if they are under 25 MB", because the unqualified sentence became false. State: 0078 on **dev** by query (statements first, history row second; recorded md5 `d0e2c83a7744cdb0f82eece20314ef03`, 33507 bytes / 32516 chars, equal to the file's LF blob). Suite 77 **62/62**, vitest **1815 / 76**. Migration number taken with Audrey's explicit permission; **Track D moves to 0079** and the ledger says so. Handbook §12.10. |
