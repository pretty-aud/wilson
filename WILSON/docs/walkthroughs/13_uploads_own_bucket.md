...
3. Now upload an invoice of **about 30 MB** (over 25 MB but UNDER 50 MB — the
   size matters, see below) on the same line, still over quota.

   **Expected:** it is refused. Since migration 0078 (Audrey's ruling,
   2026-09-09) the exemption is bounded by size: above 25 MB a money file is
   weighed like any other.
   ⚠️ **Expect an ugly error, not the friendly one.** Below 50 MB the refusal
   comes from the database as the file lands — a raw message naming
   `petal_storage_quota_insert` — because the polite "this company has used all
   its storage" sentence only exists on the reservation path, which bodies under
   50 MB never take. That is expected here and is not a fault to report.
   ...
   ☐ ~30 MB invoice refused over quota (ugly error is expected): yes / no
