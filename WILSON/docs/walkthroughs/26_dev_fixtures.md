...
| 5 | Try something the fake studio cannot do: **Bins → Add files**, or upload a file on an asset, or download one. | A toast, top of the stack: *"Dev fixtures: Picking files from disk is not available on fixture data."* Nothing is dropped silently — every refused write says so, and the control's state rolls back the way it would after a real server error. The same happens for downloads, opening a file on disk, relinking a drive, forking or sharing an O.T.T.E.R. course, inviting someone, an admin action, a model choice in Settings, and an avatar upload on your profile (the profile's text fields save, in memory). Opening a file's preview says *unavailable* — there are no bytes — with no toast, because a read that cannot work is a capability gap, not a refused write. |
...
| 11 | Optional, the proof: `npm run build` and then `grep -rl "Salt Hours" dist/`. | **Nothing.** Twelve fixture strings were grepped against a production build (the switch name, the storage key, the studio, the film, the id prefix, the badge, the adapter names): zero hits, six asset files — and the same for `npm run build:dev`, the build Electron runs. `vite build` replaces `import.meta.env.DEV` with `false`; every seam is on a line that names it first, and the dataset is behind one dynamic import in `main.jsx` that the bundler drops as dead code. `src/dev/devFixtures.test.js` pins that shape the way `devAutoLogin.test.js` pins the sign-in modes, and greps `dist/` itself whenever one has been built. |
...
- **Not in Electron at all.** The fixtures exist only under `npm run dev`
  (Vite's dev server, in a browser). `npm run electron:dev` runs
  `vite build --mode development`, and to Vite every build is a production
  build with `import.meta.env.DEV` false — measured by review round 1: a
  development-mode build contains none of the fixture strings either. So the
  desktop app, packaged or not, never carries this.
