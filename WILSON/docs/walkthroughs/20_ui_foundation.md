...
| 7 | **R.A.B.B.I.T.** → **BINS**. Open a bin, right-click a file, open **Add files**, **Delete bin**, **Assign to shot**. | **Unchanged**, on your word ("Bins waits for its own session"): the Bins tab and the Scenes takes panel keep today's look, in Geist, until B6. For a few hours tonight they ran on the shared kit (sentence-case labels, 28/36px buttons, 3px/6px corners, dialogs that fade in); that is reverted, and B6 brings it back deliberately with a walkthrough. The kit itself is built to your rulings: Dialog carries Escape, the modal stack and the busy lock by default; the backdrop click is opt-in, per your "keep it minimal". |
| 8 | Bins: hover a button. | Still the dead hover (an inline background beats the hover class) — B6's, with the state extraction. The kit keeps state in data attributes and CSS, so that class of bug cannot recur in kit components. |
...
None. Everything you ruled on tonight is in, including "Bins waits for its
own session" (step 7).
