...
| ai-proxy | Edge Function, all 3 envs; per-workspace key (`workspace_ai_keys`, AES-256-GCM) falling back to the platform `ANTHROPIC_API_KEY`. 🚨 **Verified 2026-07-30 (S17): the platform key is set on NONE of dev/staging/prod, so every AI feature returns 501 everywhere, including beta.** Earlier text here and in OWED §6B claimed staging had it; both were wrong and neither had been re-checked. **Three stores share the name and none feeds another:** GitHub *repository* secrets (Actions only — a key sits there today that no workflow even references), Vercel env vars (web build), and **Supabase Edge Function secrets, which is the one `ai-proxy` reads**. `WILSON_AI_KEY_SECRET` also owed on all three (§9C). `AI_PROXY_RPM` optional (default 60). |
