...
0028 + 0029; S17 added 0030). 🚨 **0031 (S20) is committed and deployed
NOWHERE** — it has only ever run inside rolled-back transactions against
wilson-dev. **Edge Functions:** the twelve pre-S20 ones are on all three envs;
S17 redeployed `invite-member` (it now bundles `adminGuard`). S20 added a
thirteenth, `operator-models`, **on wilson-dev only**, and deployed the S19
`ai-proxy` to wilson-dev — **prod's `ai-proxy` still silently drops
